...
const ProjectPage = () => {
  ...
  return (
    ...
    <Container>
        ...
        <Col lg={12} md={12} sm={12} xs={12} className='d-flex flex-wrap mt-3 mb-4'>
            ...
            {(!projectSelector) ? <Col style={{fontSize: '18px', fontWeight: '600', textAlign: 'center', height: 'max-content'}}>Loading...</Col> : (projectSelector.length < 1) ? <Col style={{textAlign: 'center'}}>Список пуст</Col> : projectSelector.map((card: any, index) => {
              return <Col key={index + 1} lg={3} md={3} sm={12} xs={12} className='mt-4 mb-4 d-flex justify-content-center align-items-center'><Link to={`/project/${card.id}`}><ProjectCard title={card.title} description={card.description} img={card.image}/></Link></Col>
            })}

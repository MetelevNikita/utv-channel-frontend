...
const NewsMainBlock: FC<NewsMainBlockProps> = ({ newsTitle, newsButton, linkButton, newsArr, video})  =>  {
  ...
  return (
      ...
      <Col md={12} sm={12} xs={12} className='d-flex justify-content-center align-items-center' style={{height:  '46px', width: '90%', backgroundColor: '#FEA633'}}  >
      ...
      <Col style={{height: 'max-content', width: '90%'}}>
          ...
          {newsArr.map((news: any, index: any) =>  {
            return <Col key={index} style={{width: '100%', height: 'max-content', overflow: 'hidden'}} className='d-flex mt-3 mb-2'><Link to={`/news/${news.id}`} key={index}><NewsMainPreviewCard image={news.image_1} title={news.title} lead={news.lead} date={news.date} author={news.author} video={news.video} imgHeight={'max-content'} imgWidth={'max-content'}/></Link></Col>
          })}

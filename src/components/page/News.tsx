...
const News: FC<NewsProps> = ({ modalOpen }) => {
  ...
  const buttonArr = [
...

    {
      ...
      onClick: () => {setModalNewsOpen((prev: any) => !prev)}
      ...
      <Col md={12} sm={12} xs={12} className='d-flex flex-md-row flex-column justify-content-around  mb-5'>
        ...
        <Col md={12} sm={12} xs={12} className='d-flex flex-md-row flex-column justify-content-around mt-4'>
            ...
            {buttonArr.map((button, index) => {
              return <Col key={index} md={6} sm={12} xs={12} onClick={button.onClick} className='mb-4 d-flex justify-content-between'><MyButtonLink style={{width: '90%', height: '116px', textDecoration: 'none', fontSize: '19px', color: 'black', border: '1px solid #FEA633'}} linkTitle={button.title} linkImg={button.img}/></Col>
            })}
